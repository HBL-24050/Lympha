...
class CodeBERTAnomalyParser:
    ...
    async def load(self) -> None:
        ...
        log.info(
            "Loading CodeBERT from %s on device=%s",
        ...
        try:
            ...
            log.info("CodeBERT loaded successfully on %s", resolved_device)
        except Exception as exc:
            log.warning(
                "Failed to load CodeBERT (%s); falling back to dummy scorer: %s",
                exc, self.model_name,
            )
    ...
    def analyze(self, text: str) -> tuple[float, dict]:
        ...
        try:
            ...
            if isinstance(result, list) and isinstance(result[0], dict):
                ...
                anomaly_score = 0.0
            return anomaly_score, {"method": "codebert", "raw": result}
        except Exception as exc:
            log.error("CodeBERT inference failed: %s", exc)
